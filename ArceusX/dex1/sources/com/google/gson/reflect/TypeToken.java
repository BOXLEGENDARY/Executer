...
public class TypeToken<T> {
        ...
        for (int i7 = 0; i7 < length; i7++) {
            ...
            for (Type type4 : typeVariable.getBounds()) {
                if (!C$Gson$Types.getRawType(type4).isAssignableFrom(rawType)) {
                    throw new IllegalArgumentException("Type argument " + type3 + sMlEMqrxoGI.oEUrY + typeVariable + " declared by " + type);
                }
            }
        }
    ...
    private static void verifyNoTypeVariable(Type type) {
        ...
        if (type instanceof ParameterizedType) {
            ...
            while (i7 < length) {
                verifyNoTypeVariable(actualTypeArguments[i7]);
                i7++;
            }
            return;
        }
        ...
        while (i7 < length2) {
            verifyNoTypeVariable(upperBounds[i7]);
            i7++;
        }
    }
