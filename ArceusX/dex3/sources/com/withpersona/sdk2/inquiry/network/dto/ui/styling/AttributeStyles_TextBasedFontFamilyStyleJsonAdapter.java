package com.withpersona.sdk2.inquiry.network.dto.ui.styling;

import com.squareup.moshi.h;
import com.squareup.moshi.m;
import com.squareup.moshi.t;
import com.squareup.moshi.w;
import com.withpersona.sdk2.inquiry.network.dto.ui.styling.AttributeStyles;
import com.withpersona.sdk2.inquiry.network.dto.ui.styling.StyleElements;
import kotlin.collections.L;
import org.jetbrains.annotations.NotNull;

public final class AttributeStyles_TextBasedFontFamilyStyleJsonAdapter extends h<AttributeStyles.TextBasedFontFamilyStyle> {

    @NotNull
    private final h<StyleElements.FontName> nullableFontNameAdapter;

    @NotNull
    private final m.b options = m.b.a(new String[]{"base"});

    public AttributeStyles_TextBasedFontFamilyStyleJsonAdapter(@NotNull w wVar) {
        this.nullableFontNameAdapter = wVar.f(StyleElements.FontName.class, L.d(), "base");
    }

    @NotNull
    public String toString() {
        StringBuilder sb = new StringBuilder(62);
        sb.append("GeneratedJsonAdapter(");
        sb.append("AttributeStyles.TextBasedFontFamilyStyle");
        sb.append(')');
        return sb.toString();
    }

    @NotNull
    public AttributeStyles.TextBasedFontFamilyStyle m431fromJson(@NotNull m mVar) {
        mVar.e();
        StyleElements.FontName fontName = null;
        while (mVar.u()) {
            int iP0 = mVar.P0(this.options);
            if (iP0 == -1) {
                mVar.b1();
                mVar.f1();
            } else if (iP0 == 0) {
                fontName = (StyleElements.FontName) this.nullableFontNameAdapter.fromJson(mVar);
            }
        }
        mVar.j();
        return new AttributeStyles.TextBasedFontFamilyStyle(fontName);
    }

    public void toJson(@NotNull t tVar, AttributeStyles.TextBasedFontFamilyStyle textBasedFontFamilyStyle) {
        if (textBasedFontFamilyStyle == null) {
            throw new NullPointerException("value_ was null! Wrap in .nullSafe() to write nullable values.");
        }
        tVar.f();
        tVar.Z("base");
        this.nullableFontNameAdapter.toJson(tVar, textBasedFontFamilyStyle.getBase());
        tVar.y();
    }
}
